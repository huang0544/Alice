   ...
   ================================================== -->
		...
		<div>
			<table border="1" width="100%">
				...
				<c:forEach items="${DatingManage}" var="node">
					<tr>
						<td>
							<a href="${pageContext.request.contextPath}/DatingByUserDetail?joinId=${node.joinId }&joinDatingName=${node.joinDatingName}">
								<c:out value="${node.joinDatingName}"></c:out>
							</a>
						</td>
						<td>
							<c:out value="${node.joinDatingSponsorName }"></c:out>
						</td>
						<td>
							<c:out value="${node.joinDatingCreatetime}"></c:out>
						</td>
					</tr>
				</c:forEach>
			</table>
